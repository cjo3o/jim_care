...
const dbConnect = supabase.createClient('https://orlysaskqothrdhuptkp.supabase.co',
    'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9ybHlzYXNrcW90aHJkaHVwdGtwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Mzk0MjUwNDIsImV4cCI6MjA1NTAwMTA0Mn0.06kedjSaEYJqf2ocMGsXgmmEh_38CkFWL_8CmL8MBdU');
...
$submitReserve.addEventListener('click', async function (e) {
    const res = await dbConnect.from('reservation').insert([
        {
            use_date: $dateOutput.innerHTML, // 예약 날짜 (YYYY-MM-DD)
            departure_time: $jimStartHour, // 출발 시간 (HH:MM:SS)
            departure_location: 'Seoul Station', // 출발 장소
            arrival_time: $jimEndHour, // 도착 시간 (HH:MM:SS)
            arrival_location: 'Busan Station', // 도착 장소
            shopping_bag_count: $keepBag1.value, // 쇼핑백 개수
            carrier_small_count: $keepBag2.value, // 작은 캐리어 개수
            carrier_large_count: $keepBag3.value, // 큰 캐리어 개수
            other_items: 'Umbrella, Backpack', // 기타 물품
            inquiries: $commentOutput.innerHTML, // 문의 사항
